 ...
 * @author Mirko
 */
public class ZapamtiDnevniRasporedIzlagac extends AbstractGenericOperation {

    @Override
    protected void preconditions(Object param) throws DnevniRasporedIzlagacException {
        if (param == null || (!((List<DR_I>) param).isEmpty() && !(((List<DR_I>) param).get(0) instanceof DR_I))) {
    ...
    protected void executeOperation(Object param) throws CRUDException {
        if (!((List<DR_I>) param).isEmpty()) {
            repository.Obrisi(((List<DR_I>) param).get(0));
